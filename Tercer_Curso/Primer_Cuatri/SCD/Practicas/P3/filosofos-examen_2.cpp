...
void funcion_camarero(){
    ...
    int valor;
    MPI_Status estado;
    int flag_n=0,flag_4=0;
    bool procesa_sentar=false;
    while (true){
        ...
        if(cuenta < 4){
            ...
            while (flag_n==0){
                ...
                if(procesa_sentar){
                    MPI_Iprobe(MPI_ANY_SOURCE,tag_sienta,MPI_COMM_WORLD,&flag_n,&estado);
                }else{
                    MPI_Iprobe(MPI_ANY_SOURCE,tag_levanta,MPI_COMM_WORLD,&flag_n,&estado);
                }
            }
            ...
            flag_n=0;
            flag_4=0;
            while (flag_n==0 && flag_4==0){
                procesa_sentar=!procesa_sentar;
                if(procesa_sentar){
                    MPI_Iprobe(MPI_ANY_SOURCE,tag_sienta_4,MPI_COMM_WORLD,&flag_4,&estado);
                }else{
                    MPI_Iprobe(MPI_ANY_SOURCE,tag_levanta,MPI_COMM_WORLD,&flag_n,&estado);
                }
            }
