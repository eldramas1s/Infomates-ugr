...
# Clase para representar cada estado del puzzle
class Nodo:
...
# Función para verificar si la configuración es resoluble
def es_resoluble(configuracion):
    inversiones = 0
    for i in range(len(configuracion)):
        if configuracion[i] == '0':
            continue
        for j in range(i + 1, len(configuracion)):
            if configuracion[j] == '0':
                continue
            if int(configuracion[i]) > int(configuracion[j]):
                inversiones += 1
    posicion_cero = configuracion.index('0')
    fila_cero = posicion_cero // 3
    if (inversiones % 2 == 0 and fila_cero % 2 != 0) or (inversiones % 2 != 0 and fila_cero % 2 == 0):
        return True
    return False
...
def main():
    ...
    if not es_resoluble(estado_inicial):
        print("La configuración no es resoluble.")
        sys.exit(1)
