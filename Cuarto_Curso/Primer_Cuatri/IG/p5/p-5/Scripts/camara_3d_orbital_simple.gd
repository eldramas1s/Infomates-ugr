...
var figures = ["Plano","Cilindro","Cubo"] ## mallas que podemos seleccionar
...
func select_figure_by_name(name):
	var root = get_tree().get_root().get_child(0)  # Nodo raíz de escena
	var node = root.find_child(name, true, false)
	if node:
		select_figure(node)
	else:
		print("ERROR: No se encontró figura: ", name)
