...
// This doesn't work if representation changes (now it should)
...
bool Image::Save (const char * file_path) const {
    ...
    for(int r = 0; r < rows*cols; ++r) p[r] = get_pixel(r);

    bool resultado = WritePGMImage(file_path, p, rows, cols);
