...
.section .data
lista: 		.int 1,2,3,4,5,6,7,8,9 		# ejs. binario 0b / hex 0x
...
trabajar:
	...
	call media 			    # llamar suma(&lista, longlista);
	mov %rax, resultado 	# salvar resultado
#	mov %rdx, resultado+4	# Añadir acarreo
	ret
...
media:
    ...
    mov %rax, %r9           # movemos el numerador al registro r8
    mov %ecx,%edx           # guardamos en edx el denominador
    idiv %r9                # hacemos la division y guardamos en rax
    ret
suma:
...
bucle:
	add (%rbx,%r8,4), %eax
	jnc carry		        # si no hay acarreo saltamos a carry
	inc %edx		        # incremento acarreo

carry:
	inc %r8 		        # incrementar índice
	cmp %r8,%rcx 		    # comparar con longitud
	jne bucle
ret
imprim_C:
