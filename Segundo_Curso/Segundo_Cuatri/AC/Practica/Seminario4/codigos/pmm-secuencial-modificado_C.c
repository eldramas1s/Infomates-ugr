...
int main(int argc, char** argv){
    ...
    //Trasponemos la matriz 2 haciendo que se aproveche de forma sencilla los principios de localidad.
    for(i=0; i< N; i++){
    	for(j=i+1; j < N; j++){
	   //No necesitamos comprobar indices pues i!=j para todo i,j
	   m2[i][j] = aux;
	   m2[i][j] = m2[j][i];
	   m2[j][i] = aux;
	}
