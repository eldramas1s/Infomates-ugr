...
int main(int argc, char** argv){
    ...
    #pragma omp parallel private(i) shared(m,v1,v2)
    {
        for(i = 0; i < N; i++){
            #pragma omp for firstprivate(suma_parcial) private(j) reduction(+:v2[i])
            for (j = 0; j < N; j++){
                v2[i] += m[i][j] * v1[j];
            }
            #ifdef DEBUG
    ...
    if (N < 10){
        ...
        printf("Tiempo: %11.9f \t Tamaño: %u \t Numero de hilos: %d v2[0]: %8.6f \t v2[N-1]: %8.6f \n",ncgt, N,hilos, v2[0], v2[N-1]);
    }
    return(0);
}